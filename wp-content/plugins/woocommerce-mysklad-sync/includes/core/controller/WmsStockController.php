...
class WmsStockController
{
    ...
    public function start_stock_syn()
    {
        ...
        // sleep(2);
        // WmsLogs::set_logs("Пауза 5 секунд", true);
        ...
        // WmsLogs::set_logs("Загрузка изображений из images" , true);
        ...
        WmsLogs::set_logs("Начинаем загрузку изображений из папки images", true);

        // Проверяем наличие директории
        if (!file_exists($images_dir)) {
            WmsLogs::set_logs("Папка images не найдена", true);
        }

        // Получаем список папок (каждая — это SKU)
        $sku_folders = glob($images_dir . '*', GLOB_ONLYDIR);

        foreach ($sku_folders as $sku_folder) {
            $sku = basename($sku_folder);

            // Ищем товар по SKU
            $product_id = wc_get_product_id_by_sku($sku);
            if (!$product_id) {
                WmsLogs::set_logs("Товар с SKU $sku не найден", true);
                continue;
            }

            $product = wc_get_product($product_id);
            if (!$product) {
                WmsLogs::set_logs("Ошибка при загрузке товара с ID $product_id", true);
                continue;
            }

            // Получаем изображения в папке
            $image_files = glob($sku_folder . '/*.{jpg,jpeg,png,gif}', GLOB_BRACE);
            if (empty($image_files)) {
                WmsLogs::set_logs("Нет изображений в папке $sku", true);
                continue;
            }

            // Удаляем старые изображения
            $old_image_id = $product->get_image_id();
            if ($old_image_id) wp_delete_attachment($old_image_id, true);

            $old_gallery_ids = $product->get_gallery_image_ids();
            foreach ($old_gallery_ids as $old_id) {
            ...
            foreach ($image_files as $index => $image_path) {
                ...
                if (!$upload_file['error']) {
                    ...
                    WmsLogs::set_logs("Добавлено изображение $filename к товару $sku", true);
                }
            }
            ...
            if (is_dir($sku_folder) && count(glob($sku_folder . '/*')) === 0) {
                rmdir($sku_folder);
            }

            WmsLogs::set_logs("Завершена загрузка изображений для SKU $sku", true);
        }

        WmsLogs::set_logs("Обработка изображений завершена", true);
